...
public:
    ...
    bool readData(uint16_t len, uint8_t* buffer);
    ...
    uint8_t finitepiSendData(uint8_t* params, uint16_t len, uint8_t* buffer, uint8_t wait = 5);
    bool activeTypeA(uint8_t *buffer, uint8_t kind);
    /*
   * Helper functions
   */
public:
